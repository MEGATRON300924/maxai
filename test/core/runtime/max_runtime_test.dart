...
import 'package:maxai/services/max_memory_service.dart';
...
void main() {
  test('initializes shared services and exposes a runtime snapshot', () async {
    ...
    final runtime = MaxRuntime(auth: auth, memory: memory, sync: sync, subscriptions: subscriptions);
...
class _FakeAuth implements MaxAuthContract {
  ...
  @override
  Future<MaxAuthUser?> currentUser() async => const MaxAuthUser(id: 'test-user', email: 'test@example.com');
  @override
  Future<MaxAuthResult> refreshSession() async { refreshed = true; return MaxAuthResult.success(user: await currentUser()); }
  @override
  Future<MaxAuthResult> signIn({required String email, required String password}) async => MaxAuthResult.success(user: await currentUser());
  @override
  Future<MaxAuthResult> signUp({required String email, required String password}) async => MaxAuthResult.success(user: await currentUser());
  @override
...
class _FakeMemory implements MaxMemoryContract {
  ...
  _FakeMemory() {
    ...
    _items.addAll([
      MaxMemory(id: '1', content: 'one', category: 'test', createdAt: now, updatedAt: now, importance: 1, tags: const []),
      MaxMemory(id: '2', content: 'two', category: 'test', createdAt: now, updatedAt: now, importance: 0.8, tags: const []),
    ]);
  }
  ...
  @override
  List<MaxMemory> get importantMemories => _items.where((item) => item.importance >= 0.7).toList();
  @override
  Future<MaxMemory?> save({required String content, String category = 'general', double importance = 0.5, List<String> tags = const []}) async => null;
  @override
  ...
  @override
  Future<bool> update(String id, {String? content, String? category, double? importance, List<String>? tags}) async => false;
  @override
...
class _FakeSubscriptions implements MaxSubscriptionContract {
  ...
  @override
  Future<void> refresh() async { refreshed = true; }
}
