...
class MaxMemoryService {
  ...
  List<MaxMemory> get memories{final r=List<MaxMemory>.from(_memories)..sort((a,b)=>b.updatedAt.compareTo(a.updatedAt));return List.unmodifiable(r);}
  Future<List<MaxMemory>> getMemories({String? userId}) async{await initialize();return memories;}
  List<MaxMemory> get importantMemories{final r=_memories.where((m)=>m.importance>=.7).toList()..sort((a,b)=>b.importance.compareTo(a.importance));return List.unmodifiable(r);}
  Future<MaxMemory?> save({required String content,String category='general',double importance=.5,List<String> tags=const []}) async{await initialize();final n=content.trim();if(n.isEmpty)return null;final i=_findSimilarMemory(n);if(i!=-1){final old=_memories[i];final u=old.copyWith(updatedAt:DateTime.now(),importance:importance>old.importance?importance.clamp(0,1):old.importance,tags:_mergeTags(old.tags,tags));_memories[i]=u;await _persist();return u;}final now=DateTime.now();final m=MaxMemory(id:now.microsecondsSinceEpoch.toString(),content:n,category:category.trim().isEmpty?'general':category.trim(),createdAt:now,updatedAt:now,importance:importance.clamp(0,1),tags:tags.map((e)=>e.trim().toLowerCase()).where((e)=>e.isNotEmpty).toSet().toList());_memories.add(m);await _persist();return m;}
  Future<MaxMemory?> saveMemory({String? content,String category='general',num importance=.5,List<String> tags=const [],String? userId,String? type,String? key,String? value})=>save(content:(content??value??key??'').trim(),category:type??category,importance:importance.toDouble().clamp(0,1),tags:tags);
