...
class MaxMemory {
  ...
  factory MaxMemory.fromJson(Map<String, dynamic> json) {
    ...
    return MaxMemory(
      ...
      tags: (json['tags'] as List?)?.map((tag) => tag.toString()).toList() ?? const [],
    );
  }
...
/// Local-first memory store used by every MAX surface until cloud memory is connected.
class MaxMemoryService {
    ...
    if (stored != null && stored.isNotEmpty) {
      try {
        ...
        if (decoded is List) {
          _memories
            ..clear()
            ..addAll(decoded.whereType<Map>().map((item) => MaxMemory.fromJson(Map<String, dynamic>.from(item))).where((m) => m.content.trim().isNotEmpty));
        }
      } catch (_) {
        _memories.clear();
      }
  ...
  List<MaxMemory> get importantMemories {
    final result = _memories.where((m) => m.importance >= 0.7).toList()
  ...
  Future<MaxMemory?> save({
    ...
    if (existingIndex != -1) {
      ...
      final updated = existing.copyWith(
        ...
        importance: importance > existing.importance ? importance.clamp(0.0, 1.0) : existing.importance,
    ...
    final memory = MaxMemory(
      ...
      tags: tags.map((tag) => tag.trim().toLowerCase()).where((tag) => tag.isNotEmpty).toSet().toList(),
    );
  ...
  Future<void> saveConversationMemory({required String content, double importance = 0.6, List<String> tags = const ['conversation']}) =>
      save(content: content, category: 'conversation', importance: importance, tags: tags);

  Future<void> saveVoiceMemory({required String content, double importance = 0.65, List<String> tags = const ['voice']}) =>
      save(content: content, category: 'voice', importance: importance, tags: tags);

  Future<void> savePreference({required String content, List<String> tags = const ['preference']}) =>
      save(content: content, category: 'preference', importance: 0.9, tags: tags);

  Future<void> saveImportantFact({required String content, List<String> tags = const ['important']}) =>
      save(content: content, category: 'important', importance: 1.0, tags: tags);
  ...
  List<MaxMemory> search(String query, {int limit = 20}) {
    ...
    for (final memory in _memories) {
      ...
      for (final term in terms) {
        if (content.contains(term)) score += 3.0;
        if (category.contains(term)) score += 2.0;
        if (tags.contains(term)) score += 2.5;
      }
  ...
  List<MaxMemory> relevantMemories(String query, {int limit = 8}) {
    ...
    return results.isNotEmpty ? results : importantMemories.take(limit).toList();
  }

  String buildMemoryContext(String query, {int limit = 8}) {
    return relevantMemories(query, limit: limit).map((memory) => '- ${memory.content}').join('\n');
  }

  Future<bool> update(String id, {String? content, String? category, double? importance, List<String>? tags}) async {
    await initialize();
    final index = _memories.indexWhere((memory) => memory.id == id);
    if (index == -1) return false;
    _memories[index] = _memories[index].copyWith(content: content, category: category, importance: importance, tags: tags, updatedAt: DateTime.now());
    await _persist();
    return true;
  }
  ...
  Future<bool> delete(String id) async {
    ...
    if (_memories.length == before) return false;
    await _persist();
    return true;
  }
  ...
  Future<void> clear({bool keepImportant = true}) async {
    await initialize();
    if (keepImportant) {
      _memories.removeWhere((memory) => memory.importance < 0.9);
    } else {
      _memories.clear();
    }
    await _persist();
  ...
  Future<void> importMemories(List<Map<String, dynamic>> data) async {
    ...
    for (final item in data) {
      try {
        ...
        final index = _memories.indexWhere((existing) => existing.id == memory.id);
        if (index == -1) {
          _memories.add(memory);
        } else if (memory.updatedAt.isAfter(_memories[index].updatedAt)) {
  ...
  int _findSimilarMemory(String content) {
    ...
    for (var i = 0; i < _memories.length; i++) {
      final existing = _normalize(_memories[i].content);
      if (existing == normalized || _similarity(existing, normalized) >= 0.92) return i;
    }
    return -1;
  }
  ...
  double _similarity(String first, String second) {
    ...
    return union == 0 ? 0 : firstWords.intersection(secondWords).length / union;
  }
  ...
  Future<void> _persist() async {
    _preferences ??= await SharedPreferences.getInstance();
    await _preferences!.setString(_storageKey, jsonEncode(_memories.map((memory) => memory.toJson()).toList()));
  }
}
...
class _MemoryScore {
  ...
  final double score;
  const _MemoryScore({required this.memory, required this.score});
}
